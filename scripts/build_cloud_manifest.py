...
This script scans an R2 bucket and creates a metadata.csv file with public URLs
based on a flexible configuration of asset types.
...
def build_cloud_manifest_from_track_ids(track_ids: list[str], output_path: Path):
    ...
    print(f"Building manifest for track IDs: {track_ids}")
...
def process_paths(all_paths: list[str], output_path: Path) -> bool:
    """Process a list of paths and create the manifest."""
    assets_by_track = {}
    ...
    print(f"\nCloud manifest with {len(manifest_df)} tracks written to: {output_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Generate a cloud dataset manifest.")
    parser.add_argument(
        "--track-ids",
        type=str,
    ...
    parser.add_argument(
        ...
        "-o",
        type=Path,
        default=Path("manifest_cloud.csv"),
        help="Output path for manifest CSV (default: manifest_cloud.csv)",
    )
